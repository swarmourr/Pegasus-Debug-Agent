...
[
 { "type": "transfer",
   "linkage": "input",
   "lfn": "BRS2_R2.fastq.gz",
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./BRS2_R2.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/BRS2_R2.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
   "lfn": "BRS2_R1.fastq.gz",
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./BRS2_R1.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/BRS2_R1.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
...
BRS2_R2.fastq.gz;;;BRS2_R1.fastq.gz;;;trim_reads
eof
...
cat <<EOF > trim_reads_trim_BRS2-cont.sh
#!/bin/bash
set -e

# setting environment variables for job
EOF
container_env /scratch >> trim_reads_trim_BRS2-cont.sh
cat <<EOF2 >> trim_reads_trim_BRS2-cont.sh
...
pegasus-kickstart  -n trim_reads -N trim_BRS2 -R compute  -S @$_CONDOR_SCRATCH_DIR/trim_reads_trim_BRS2.in.lof  -s @$_CONDOR_SCRATCH_DIR/trim_reads_trim_BRS2.out.lof  -L subductcr-end-to-end -T 2026-09-11T05:13:20+00:00 -k 42900 -K 150 ./trim_reads BRS2_R1.fastq.gz BRS2_R2.fastq.gz BRS2_trim_R1.fastq.gz BRS2_trim_R2.fastq.gz
EOF2


chmod +x trim_reads_trim_BRS2-cont.sh
...
shifter --image swarmourr/subductcr-mothur:latest --volume $PWD:/scratch --workdir=/scratch ./trim_reads_trim_BRS2-cont.sh 
job_ec=$(($job_ec + $?))
...
[
 { "type": "transfer",
   "linkage": "output",
   "lfn": "BRS2_trim_R1.fastq.gz",
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/BRS2_trim_R1.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./BRS2_trim_R1.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "output",
   "lfn": "BRS2_trim_R2.fastq.gz",
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/BRS2_trim_R2.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./BRS2_trim_R2.fastq.gz" }
   ] }
]
eof
