...
[
 { "type": "transfer",
   ...
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./SLS_R1.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/SLS_R1.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
   ...
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./SLS_R2.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/SLS_R2.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
...
SLS_R1.fastq.gz;;;SLS_R2.fastq.gz;;;trim_reads
eof
...
cat <<EOF > trim_reads_trim_SLS-cont.sh
#!/bin/bash
set -e

# setting environment variables for job
EOF
container_env /scratch >> trim_reads_trim_SLS-cont.sh
cat <<EOF2 >> trim_reads_trim_SLS-cont.sh
...
pegasus-kickstart  -n trim_reads -N trim_SLS -R compute  -S @$_CONDOR_SCRATCH_DIR/trim_reads_trim_SLS.in.lof  -s @$_CONDOR_SCRATCH_DIR/trim_reads_trim_SLS.out.lof  -L subductcr-end-to-end -T 2026-09-11T05:13:20+00:00 -k 42900 -K 150 ./trim_reads SLS_R1.fastq.gz SLS_R2.fastq.gz SLS_trim_R1.fastq.gz SLS_trim_R2.fastq.gz
EOF2


chmod +x trim_reads_trim_SLS-cont.sh
...
shifter --image swarmourr/subductcr-mothur:latest --volume $PWD:/scratch --workdir=/scratch ./trim_reads_trim_SLS-cont.sh 
job_ec=$(($job_ec + $?))
...
[
 { "type": "transfer",
   "linkage": "output",
   "lfn": "SLS_trim_R1.fastq.gz",
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/SLS_trim_R1.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./SLS_trim_R1.fastq.gz" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "output",
   "lfn": "SLS_trim_R2.fastq.gz",
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/SLS_trim_R2.fastq.gz", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./SLS_trim_R2.fastq.gz" }
   ] }
]
eof
