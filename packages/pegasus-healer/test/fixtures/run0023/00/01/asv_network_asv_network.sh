...
[
 ...
 { "type": "transfer",
   ...
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./asv_network", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/asv_network" }
   ] }
]
eof
...
cat <<EOF > asv_network_asv_network-cont.sh
#!/bin/bash
set -e

# setting environment variables for job
EOF
container_env /scratch >> asv_network_asv_network-cont.sh
cat <<EOF2 >> asv_network_asv_network-cont.sh
...
pegasus-kickstart  -n asv_network -N asv_network -R compute  -S @$_CONDOR_SCRATCH_DIR/asv_network_asv_network.in.lof  -s @$_CONDOR_SCRATCH_DIR/asv_network_asv_network.out.lof  -L subductcr-end-to-end -T 2026-09-11T05:13:20+00:00 -k 42900 -K 150 ./asv_network filtered.rds asv_cliques.rds
EOF2


chmod +x asv_network_asv_network-cont.sh
...
shifter --image swarmourr/subductcr-r:latest --volume $PWD:/scratch --workdir=/scratch ./asv_network_asv_network-cont.sh 
job_ec=$(($job_ec + $?))
...
[
 { "type": "transfer",
   ...
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/asv_cliques.rds", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./asv_cliques.rds" }
   ] }
]
eof
