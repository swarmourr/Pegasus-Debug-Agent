...
[
 ...
 { "type": "transfer",
   ...
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_geochem", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_geochem" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
   "lfn": "gene_cliques.rds",
   "id": 3,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_cliques.rds", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_cliques.rds" }
   ] }
]
eof
...
cat <<EOF > gene_geochem_gene_geochem-cont.sh
#!/bin/bash
set -e

# setting environment variables for job
EOF
container_env /scratch >> gene_geochem_gene_geochem-cont.sh
cat <<EOF2 >> gene_geochem_gene_geochem-cont.sh
...
pegasus-kickstart  -n gene_geochem -N gene_geochem -R compute  -S @$_CONDOR_SCRATCH_DIR/gene_geochem_gene_geochem.in.lof  -s @$_CONDOR_SCRATCH_DIR/gene_geochem_gene_geochem.out.lof  -L subductcr-end-to-end -T 2026-09-11T05:13:20+00:00 -k 42900 -K 150 ./gene_geochem gene_cliques.rds geochem.csv gene_clique_env.tsv
EOF2


chmod +x gene_geochem_gene_geochem-cont.sh
...
shifter --image swarmourr/subductcr-r:latest --volume $PWD:/scratch --workdir=/scratch ./gene_geochem_gene_geochem-cont.sh 
job_ec=$(($job_ec + $?))
...
[
 { "type": "transfer",
   ...
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_clique_env.tsv", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_clique_env.tsv" }
   ] }
]
eof
