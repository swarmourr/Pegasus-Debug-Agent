...
[
 { "type": "transfer",
   ...
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_table.tsv", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_table.tsv" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "input",
   ...
   "id": 2,
   "src_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_network", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_network" }
   ] }
]
eof
...
cat <<EOF > gene_network_gene_network-cont.sh
#!/bin/bash
set -e

# setting environment variables for job
EOF
container_env /scratch >> gene_network_gene_network-cont.sh
cat <<EOF2 >> gene_network_gene_network-cont.sh
...
pegasus-kickstart  -n gene_network -N gene_network -R compute  -S @$_CONDOR_SCRATCH_DIR/gene_network_gene_network.in.lof  -s @$_CONDOR_SCRATCH_DIR/gene_network_gene_network.out.lof  -L subductcr-end-to-end -T 2026-09-11T05:13:20+00:00 -k 42900 -K 150 ./gene_network gene_table.tsv gene_cliques.rds
EOF2


chmod +x gene_network_gene_network-cont.sh
...
shifter --image swarmourr/subductcr-r:latest --volume $PWD:/scratch --workdir=/scratch ./gene_network_gene_network-cont.sh 
job_ec=$(($job_ec + $?))
...
[
 { "type": "transfer",
   ...
   "id": 1,
   "src_urls": [
     { "site_label": "compute", "url": "file://$PWD/gene_cliques.rds", "checkpoint": "false" }
   ],
   "dest_urls": [
     { "site_label": "compute", "url": "file:///pscratch/sd/h/hsafri/wf-scratch/hsafri/pegasus/subductcr-end-to-end/run0023/./gene_cliques.rds" }
   ] }
]
eof
